    ...
    <div className="min-h-screen bg-blue-700">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 sm:px-6 lg:px-8 pb-24 sm:pb-8">
        ...
        <main id="main-content">
          {mode === "speak" ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              >
                <svg width="48" height="42" viewBox="0 0 64 56" fill="none" aria-hidden="true">
              ...
              <div className="mt-8 max-w-xs">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {["I had two eggs", "How many calories today?", "What did I eat?", "Delete my last entry", "How's my progress?"].map((prompt) => (
                    <span key={prompt} className="bg-white/10 border border-white/15 text-white/80 px-3 py-1.5 rounded-full text-xs">
                      &ldquo;{prompt}&rdquo;
                    </span>
                  ))}
                </div>
              ...
              <section aria-labelledby="summary-heading" className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-4 sm:p-6">
                ...
                <div className="flex gap-3 mb-3">
                  <div className="bg-white/10 rounded-lg p-3 text-center min-w-[100px]">
                    <p className="text-xs text-white uppercase tracking-wide font-medium">Calories</p>
                    <p className="text-2xl font-bold text-white" aria-label={`${summary.calories} of ${calorieGoal} calories`}>
                      {summary.calories}
                      <span className="text-sm font-normal text-white">/{calorieGoal}</span>
                    </p>
                  </div>
                      ...
                      {(["protein", "carbs", "fat"] as const).map((key) => {
                        ...
                        return (
                          <label key={key} className="flex flex-col items-center gap-1 cursor-pointer">
                            ...
                            >
                              <span aria-hidden="true" className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform duration-200 ${pressed ? "translate-x-5" : "translate-x-0"}`} />
                            </button>
                          </label>
                        );
                      })}
                      ...
                      <div className="flex gap-2">
                        {showNutrients.protein && (
                          <div className="bg-white/10 rounded-lg p-2 text-center flex-1">
                            <p className="text-xs text-white uppercase tracking-wide font-medium">Protein</p>
                            <p className="text-base font-bold text-white">{Number(summary.protein).toFixed(1)}<span className="text-xs font-normal text-white">g</span></p>
                          </div>
                        )}
                        {showNutrients.carbs && (
                          <div className="bg-white/10 rounded-lg p-2 text-center flex-1">
                            <p className="text-xs text-white uppercase tracking-wide font-medium">Carbs</p>
                            <p className="text-base font-bold text-white">{Number(summary.carbs).toFixed(1)}<span className="text-xs font-normal text-white">g</span></p>
                          </div>
                        )}
                        {showNutrients.fat && (
                          <div className="bg-white/10 rounded-lg p-2 text-center flex-1">
                            <p className="text-xs text-white uppercase tracking-wide font-medium">Fat</p>
                            <p className="text-base font-bold text-white">{Number(summary.fat).toFixed(1)}<span className="text-xs font-normal text-white">g</span></p>
                          </div>
                        )}
                      </div>
                    )}
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <p className="text-xs text-white">{caloriePercent}% of daily goal</p>
                    <p className="text-xs text-white">{summary.entry_count} {summary.entry_count === 1 ? "entry" : "entries"}</p>
                  </div>
                </div>
                ...
                <details className="group">
                  ...
                  <div className="mt-3 flex flex-col gap-4 border-t border-white/20 pt-4">
                    <fieldset>
                      <legend className="text-sm font-medium text-white mb-2">Update calorie goal</legend>
              ...
              <section aria-labelledby="log-input-heading" className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-4 sm:p-7">
                ...
                <div className="flex items-center gap-3 sm:items-end sm:gap-4">
                  <div className="flex-shrink-0 flex flex-col items-center gap-1.5">
                    <span className="text-[10px] sm:text-xs text-white">Speak to me</span>
                    <button
                      type="button"
                      onClick={recording ? stopRecording : startRecording}
                      disabled={loading}
                    ...
                    >
                      <svg width="16" height="14" viewBox="0 0 64 56" fill="none" aria-hidden="true">
                        <path d="M10 10 Q10 3 16 3 Q26 3 29 14 Q32 25 29 38 Q26 49 16 49 Q10 49 10 42 L10 36 Q15 39 20 36 Q27 33 27 22 Q27 11 20 8 Q15 6 13 10 Z" stroke="white" strokeWidth="2" fill="none" strokeLinejoin="round" />
                        <path d="M13 25 Q11 29 13 33" stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" />
                        <path d="M36 12 Q44 28 36 44" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.85" />
                        <path d="M43 7 Q54 28 43 49" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.55" />
                  ...
                  <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                    <label htmlFor="food-text-input" className="text-[10px] sm:text-xs text-white">
                      Type it instead
                    </label>
              ...
              {pendingParse && (
                ...
                >
                  ...
                  {pendingParse.parsed.reasoning && (
                    <p className="text-white text-sm mb-3">{pendingParse.parsed.reasoning}</p>
                  )}
                  {pendingParse.parsed.alternatives && pendingParse.parsed.alternatives.length > 0 && (
                    <div className="mb-4">
                      <p className="text-xs text-white uppercase tracking-wide font-medium mb-2">Did you mean?</p>
                  ...
                  <div id="logs-table" className="mt-3">
                    {logs.length === 0 ? (
                      <div className="bg-white/10 border border-white/20 rounded-xl p-8 text-center">
                        <p className="text-white text-sm">Nothing logged yet today.</p>
                          ...
                          <table className="w-full min-w-[500px] text-sm text-left" aria-label="Today's food log entries">
                            <thead>
                              <tr className="border-b border-white/20 bg-white/10">
                                <th scope="col" className="px-3 py-2 text-[10px] font-semibold text-white uppercase tracking-wide">Food</th>
                                <th scope="col" className="px-3 py-2 text-[10px] font-semibold text-white uppercase tracking-wide">Cal</th>
                                <th scope="col" className="px-3 py-2 text-[10px] font-semibold text-white uppercase tracking-wide">Protein</th>
                                <th scope="col" className="px-3 py-2 text-[10px] font-semibold text-white uppercase tracking-wide">Carbs</th>
                                <th scope="col" className="px-3 py-2 text-[10px] font-semibold text-white uppercase tracking-wide">Fat</th>
        ...
        <nav aria-label="Main navigation" className="fixed bottom-0 left-0 right-0 bg-blue-800 border-t border-white/20 flex justify-around items-center py-2 sm:hidden">
          <button type="button" aria-label="Home" className="flex flex-col items-center gap-1 p-2 text-white/80 hover:text-white transition-colors">
          ...
          <button type="button" aria-label="Progress" className="flex flex-col items-center gap-1 p-2 text-white/80 hover:text-white transition-colors">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <button type="button" aria-label="History" className="flex flex-col items-center gap-1 p-2 text-white/80 hover:text-white transition-colors">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <button type="button" aria-label="Profile" className="flex flex-col items-center gap-1 p-2 text-white/80 hover:text-white transition-colors">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
