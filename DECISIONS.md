...
- **2026-09-06 — Passive calorie-macro mismatch marker at 15% relative error; do not block logs or rerank.** USDA branded source data is internally inconsistent on ~9% of rows and nothing identifies which of the four numbers is wrong, so lookup stamps `calorie_macro_mismatch` on match metadata and leaves ranking and logging unchanged.
