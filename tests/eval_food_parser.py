...
EVAL_CASES: list[EvalCase] = [
    # --- simple (8) ---
    EvalCase("an apple", "apple", "1", "medium", "simple"),
    ...
    EvalCase("a big bowl of rice", "rice", "1", "bowl", "vague"),
    EvalCase("some pasta", "pasta", "1", "serving", "vague"),
    EvalCase("a little butter", "butter", "1", "tablespoon", "vague"),
    EvalCase("a few grapes", "grapes", "few", "", "vague"),
    EvalCase(
...
def parse_serving_size(serving: str | None) -> tuple[str | None, str | None]:
    ...
    match = re.match(
        ...
        r"half|quarter|couple|few|some|little|big|small|large|medium|handful|scoop)\s+(.+)$",
        text,
    )
    if match:
        return _normalize_quantity(match.group(1)), match.group(2).strip()

    if re.match(r"^\d+(?:\.\d+)?$", text) or text in _WORD_TO_NUM or text in _VAGUE_QUANTITIES:
...
def format_report(report: dict) -> str:
    ...
    for failure in failures:
        ...
        if failure.error:
            ...
            lines.append(
                f"    expected: food={case.expected_food!r} "
                f"qty={case.expected_quantity!r} unit={case.expected_unit!r}"
            )
            lines.append(
            ...
            lines.append(
                f"    scores:   food_exact={failure.food_exact} "
                f"food_fuzzy={failure.food_fuzzy_ratio:.2f} "
                f"qty={failure.quantity_match} unit={failure.unit_match} "
                f"case={failure.case_score:.1%}"
            )
